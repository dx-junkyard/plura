...
提供されたドキュメントを読み、単なる結論の羅列ではなく、「著者がたどり着いた思考のプロセス」を重視して要約してください。

## 出力フォーマット
以下の構成で、箇条書きや見出しを使って構造的にまとめてください。

### 背景と課題
- このドキュメントが何を解決しようとしているか

### 検討プロセスとジレンマ（該当する場合）
- 検討された選択肢、トレードオフ、迷いのポイント
- ※ドキュメントに検討プロセスが含まれていない場合はこのセクションを省略する
...
### その他の重要ポイント
- 上記に収まらない重要な事実・数値・定義など

## ルール
- 提供されたドキュメントの内容のみを根拠にする
- 専門用語は簡潔に補足する
- 日本語で回答する
...
以下はドキュメント全体の一部（セクション）です。このセクション内の内容を中間要約として整理してください。

**特に以下の情報が含まれている場合は必ず抽出・保持してください：**
- 背景や課題（何を解決しようとしているか）
- 検討の過程での迷い・ジレンマ・トレードオフ（AかBか、という葛藤）
- 結論に至った理由・プロセス（なぜその選択をしたか）
...
これらを統合し、ドキュメント全体の「生きた知恵」として後から見た人がすぐに活用できる最終要約を生成してください。
...
### 検討プロセスとジレンマ（該当する場合）
- 検討された選択肢、トレードオフ、迷いのポイント
- ※中間要約に検討プロセスが含まれていない場合はこのセクションを省略する
...
---
上記のドキュメントを要約してください。
単なる結論の列挙ではなく、「背景・課題」「検討プロセスやジレンマ（もし含まれていれば）」「結論とその理由」を重視して、構造的にまとめてください。"""
...
背景・課題、検討プロセス・ジレンマ（含まれていれば）、結論の理由など、思考プロセスに関わる情報を優先して抽出してください。"""
...
「背景と課題」「検討プロセスとジレンマ」「結論とその理由」の構成で、後から見た人が「生きた知恵」としてすぐに活用できるようまとめてください。"""
